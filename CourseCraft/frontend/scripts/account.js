...
function showUserData() {
  const userProfileContainer = document.getElementsByClassName("account");
  const codeName = document.getElementById("usercodeName");
  const userNameSpan = document.getElementById("userNameSpan");
  const userEmailSpan = document.getElementById("userEmailSpan");

  // Retrieve data from localStorage using the key (e.g., 'userData')
  const userData = JSON.parse(localStorage.getItem("userInfo"));

  if (userData) {
    // Update the HTML with the fetched data
    codeName.textContent = userData.name + "#123";
...
// Call the function to show user data when the page loads
showUserData();
